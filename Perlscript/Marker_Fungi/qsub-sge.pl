...
AS in current date menbers in different groups share different compute node, -qoptions option was set to add
other qsub options, e.g: -qoptionsi="-l h=compute-0-43 ". Also it can use to appoint resource or queue, e.g:
-qoptions="-q supmen.q -P supmen -l vf=80g". In the other way, I delete the original qsub commend "-S /bin/sh",
as no long ago some annotation colleagues said it may cause problems. If necessary, you can use -qoptions="-S /bin/sh"
to add it. At last, please note that behind -qoptions must have '='. Good luck for you!!
...
=head1 Version
  ...
  Mender: Liu Wenbin, liuwenbin@genomics.cn
  Version: 8.3,  Date: 2011-04-02

=head1 Usage
  
  perl qsub-sge.pl <jobs.txt>
  --queue <str>     specify the queue to use, default all availabile queues 
  --P <str>	project name in PM system
  ...
  --qoptions <str>  set other qsub options, default not set
...
=head1 Exmple
  ...
  8.use -qoptions to appoint computer node
  perl qsub-sge.pl -maxjob 10 -qoptions " -l h=compute-0-126 -l vf=32g -q supmen.q -P supmen" ./work.sh
...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,
        $Qoptions, $Job_prefix,$Verbose, $project, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while (@Shell) {
    ...
    $job_cmd .= " $Qoptions" if(defined $Qoptions);  ##add other qsub options
	$job_cmd .= "-P $project" if (defined $project);## set the name of the project
	##warn $job_cmd;
