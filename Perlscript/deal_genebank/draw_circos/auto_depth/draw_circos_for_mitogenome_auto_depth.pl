...
# statistic all length of genome to avoid more than limitation in housekeeping conf
my $total_length = 0;
...
if ($total_length > 25000){ ##the limitation in housekeeping conf
	print STDERR "!!!\n";
	print STDERR "The line number of showing GC content reachs up to limitation setted in housekeeping configure, \
so you need to modify this setting manually => etc/housekeeping.conf\n";
	print STDERR "!!!";
	exit;
}
