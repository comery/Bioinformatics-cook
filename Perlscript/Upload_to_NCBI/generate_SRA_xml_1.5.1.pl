...
my $XSD_DIR = "/ifs2/BC_GAG/Project/Alashan_Bactrian/Evolution/For_SRA/SRA_1-5";
...
<SUBMISSION_SET xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <SUBMISSION accession="$sra_id" alias="$sra_title" center_name="$center_name" lab_name="$center_name" submission_comment="$submission_comment" submission_date="$submission_date">
    ...
    <ACTIONS>
      <ACTION>
        <ADD schema="study" source="$sra_id.study.xml"/>
      </ACTION>
      <ACTION>
        <ADD schema="sample" source="$sra_id.sample.xml"/>
      </ACTION>
      <ACTION>
...
foreach $fastq_list ( keys %difference_option ) {
	...
	while (<FASTQLIST>) {
		...
		my @F = split /\t/;#modified by guangxm
		for ( my $i=0; $i<@F; $i++ ) { $F[$i]=~s/\s//g } #clean blank space

		my @fastq;
		@fastq = <$F[10]*gz> or @fastq = <$F[10]*fq>;
		next unless @fastq;
		...
		if ( $count{$library_mark} == 1 ) {
			...
			$subexperiment .= "    <STUDY_REF accession=\"$bioproject_id\" refname=\"$sra_title\"/>\n"; ##accession & refname --> SRA061649 & bladder cancer exomes sequencing
			...
			$subexperiment .= "      <SAMPLE_DESCRIPTOR refname=\"$sample\"/>\n"; ##refname2 --> $F[7]
...
print STDERR "\n2. Validating submission.xml code......\n";
