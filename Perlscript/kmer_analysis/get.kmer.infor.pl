...
#use strict;
use File::Basename;
die "Usage:perl $0 <KMER_log> <KMER_freq> <OUT_dir> <xlim> <ylim> <Title>!"unless @ARGV >0;
my $kmerlog=shift;
my $kmerfreq=shift;
my $outdir=shift;
my $xlim=shift;
...
for($i=0;$i<=$#freq;$i++){
	if($freq[$i][0]==255){
		...
		print DATA "$freq[$i][0]\t$freq[$i-1][2]\t100\t$pro\t100\n";
	}else{
		$freq_acc+=$freq[$i][2];
		$pro=$freq[$i][3]/$C[1]*100;
		$pro_acc+=$pro;
...
print Hr "pdf(file='$outdir/infor.out.pdf',width=16,height=10)\n";
...
system "/opt/blc/genome/biosoft/R/bin/R < $outdir/kmer.draw.R --no-save\n";
