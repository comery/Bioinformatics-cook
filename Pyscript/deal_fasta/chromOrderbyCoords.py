...
def read_ref_block(inputfile):
    ...
    with open(inputfile, 'r') as fh:
        while True:
            ...
            ref_lens[chr1] = len_chr1
            que_lens[chr2] = len_chr2
    return coords, ref_lens, que_lens
...
def main():
    ...
    new_ref_seq = []
    new_que_seq = []
    out1 = open("new_ref.bed", 'w')
    out2 = open("new_que.bed", 'w')
    current_ref_start = 1
    current_que_start = 1

    coords, ref_lens, que_lens = read_ref_block(sys.argv[1])
    # sort ref scaffolds by length
    ref_chr_sorted = sorted(ref_lens.keys(), key=lambda x: ref_lens[x], reverse=True)
    for ref_chr in ref_chr_sorted:
        # store each ref sequence to a list for output
        new_ref_seq.append(ref_seqs[ref_chr])
        # to generate agp file for newly makeuped ref seq
        current_ref_end = current_ref_start + ref_lens[ref_chr] - 1
        ...
        for related_blocks in get_related_blocks(coords[ref_chr]):
            # only one block
            ...
            if b.strand >= 0:
                ...
                print(f"Que_chr1\t{current_que_start}\t{current_que_end}\t{b.chr2}\t+", file=out2)
            else:
    ...
    if ref_chr != ref_chr_sorted[-1]: #don't add gap at the end
            new_que_seq.append("n" * 500)
            current_ref_end = current_ref_start + 500 - 1 # add gap
            print(f"Ref_chr1\t{current_ref_start}\t{current_ref_end}\tgap\t+", file=out1)
            current_ref_start = current_ref_end + 1
            current_que_end = current_que_start + 500 - 1 # add gap
            print(f"Que_chr1\t{current_que_start}\t{current_que_end}\tgap\t+", file=out2)
            current_que_start = current_que_end + 1
    ...
    gap_joiner = "n" * 500
    with open("new_ref.fa", 'w') as fh:
        print(">Ref_chr1", file=fh)
        print(wrapseq(gap_joiner.join(new_ref_seq)), file=fh)
