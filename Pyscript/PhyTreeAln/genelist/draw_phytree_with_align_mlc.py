...
def draw_tre_and_aln(aln, tre_str, ps_site, og, target_sp):
    """
...
def main():
    with open(ogs_list, 'r') as fh:
        for i in fh:
            ...
            positive_sites_file = os.path.join(tmp_outdir, family_id + ".sites")
            ps = open(positive_sites_file, 'w')
            psg_sites = parse_BEB_sites(mlc_f) # write positive sites to file
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        sys.exit("python3 {} <gene_list> <specific short name for #1> <outdir|default=Results_phyaln>".format(sys.argv[0]))
    elif len(sys.argv) > 3:
        outdir = sys.argv[3]
