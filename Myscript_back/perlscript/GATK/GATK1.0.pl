...
=head1 Usage
 options:(write in main.sh )
  ...
  -genome *              genome file,muse be end of .fa or .fasta
...
if (-e '/ifs2') {
	$BinDir="/ifs4/NGB_ENV/USER/yangchentao/software/perlscript/GATK"; #
	$GATK="java -jar /ifs2/PC_PA_UN_2/USER/zhouchengran/bin/GATK-3.2.2/GenomeAnalysisTK.jar"; #/ifs4/BC_PUB/biosoft/pipeline/DNA/DNA_CSAP/DNA_CSAP_5.2.4/bin/Tool/GenomeAnalysisTK.jar;
	$BWA="/ifs4/BC_PUB/biosoft/pipeline/Package/bwa-0.7.10/bwa"; #i/ifs2/PC_PA_UN_2/USER/zhouchengran/bin/bwa-0.7.10/bwa;
	$samtoolsdir="/ifs4/BC_PUB/biosoft/pipeline/Package/samtools-1.0/"; #/ifs2/PC_PA_UN_2/USER/zhouchengran/bin/samtools-0.1.19;
	$samtools="/ifs4/BC_PUB/biosoft/pipeline/Package/samtools-1.0/samtools"; #/ifs2/PC_PA_UN_2/USER/zhouchengran/bin/samtools-0.1.19/samtools;
	$picard="/ifs4/BC_PUB/biosoft/pipeline/Package/picard-tools-1.117"; 
	$python="/ifs4/BC_PUB/biosoft/pipeline/Package/Python/Python-2.7.7/bin/python2.7";
}
...
print FINAL "perl $Bin/qsub-sge.pl  --getmem   --lines 3 --convert no --resource vf=500M --queue $queue --subprjctid ngb_un  --jobprefix INDEX $INDEXDir/bulid_index.sh\n";
print FINAL "echo -e '\\t' build index done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#BWA \n";
print FINAL "perl $Bin/qsub-sge.pl  --getmem  --maxjob 100 --convert no --reqsub --queue $queue  --subprjctid ngb_un  --resource vf=2G   --jobprefix BWA $BWADir/bwa.sh\n";
print FINAL "echo -e '\\t' bwa done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#split bwa result \n";
print FINAL "perl $Bin/qsub-sge.pl  --getmem  --maxjob 100 --convert no --reqsub --queue $queue  --subprjctid ngb_un  --resource vf=2G  --jobprefix Chrbam $ChrbamDir/Chrbam.sh\n";
print FINAL "echo -e '\\t' split done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#generate  preprocess result \n";
print FINAL "perl $Bin/qsub-sge.pl  --getmem  --maxjob 100 --convert no  --reqsub --queue $queue --subprjctid ngb_un  --resource vf=12G --jobprefix PRE $PARTSHELLDir/qsubpre.sh\n";
...
print FINAL "perl $Bin/qsub-sge.pl  --getmem  --maxjob 100 --convert no --reqsub  --queue $queue --subprjctid ngb_un --resource vf=10G   --jobprefix CALLSNP $CALLSNPshellDir/qsubcallsnp.sh\n";
print FINAL "echo -e '\\t'  generate snp result done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#call  indel and VQSR \n";
print FINAL "perl $Bin/qsub-sge.pl  --getmem  --maxjob 100 --convert no --reqsub  --queue $queue --subprjctid ngb_un  --resource vf=10G   --jobprefix CALLINDEL $CALLINDELshellDir/qsubcallindel.sh\n";
