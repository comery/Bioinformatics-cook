...
if (-e '/ifs2') {
	...
	$GATK="java -jar /ifs2/BC_GAG/Group/zhuangzhenhua/s_ware/GenomeAnalysisTK-2.4-9/GenomeAnalysisTK.jar";
	$BWA="/ifs2/BC_GAG/Group/zhuangzhenhua/s_ware/bwa-0.7.0/bwa";
	$samtoolsdir="/ifs2/BC_GAG/Group/zhuangzhenhua/s_ware/samtools-0.1.18";
	$samtools="/ifs2/BC_GAG/Group/zhuangzhenhua/s_ware/samtools-0.1.18/samtools";
...
print FINAL "perl $Bin/qsub-sge.pl   --lines 3 --convert no --resource vf=2G --queue $queue --P $project --jobprefix INDEX $INDEXDir/bulid_index.sh\n";
print FINAL "echo -e '\\t' build index done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#BWA \n";
print FINAL "perl $Bin/qsub-sge.pl  --maxjob 100 --convert no --reqsub --queue $queue --P $project --resource vf=`du -b $GENOME | awk '{sum += \$1} END {printf(\"%.2fG\", sum / 1024 ^ 3*6)}'` --jobprefix BWA $BWADir/bwa.sh\n";
print FINAL "echo -e '\\t' bwa done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#split bwa result \n";
print FINAL "perl $Bin/qsub-sge.pl  --maxjob 100 --convert no --reqsub --queue $queue --P $project --resource vf=`du -b $GENOME | awk '{sum += \$1} END {printf(\"%.2fG\", sum / 1024 ^ 3*10)}'` --jobprefix Chrbam $ChrbamDir/Chrbam.sh\n";
print FINAL "echo -e '\\t' split done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#generate  preprocess result \n";
print FINAL "perl $Bin/qsub-sge.pl  --maxjob 100 --convert no  --reqsub --queue $queue --P $project --resource vf=`du -b $GENOME | awk '{sum += \$1} END {printf(\"%.2fG\", sum / 1024 ^ 3*20)}'` --jobprefix PRE $PARTSHELLDir/qsubpre.sh\n";
print FINAL "echo -e '\\t' preprocess result done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#generate  confidence snpdb file \n";
print FINAL "perl $Bin/qsub-sge.pl  --maxjob 100  --convert no --reqsub --queue $queue --P $project --resource vf=10G --jobprefix CON $PREDir/qsubsnpdb.sh\n";
print FINAL "echo -e '\\t' generate snpdb file done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#call  snp and VQSR \n";
print FINAL "perl $Bin/qsub-sge.pl  --maxjob 100 --convert no --reqsub  --queue $queue --P $project --resource vf=`du -b $GENOME | awk '{sum += \$1} END {printf(\"%.2fG\", sum / 1024 ^ 3*20)}'` --jobprefix CALLSNP $CALLSNPshellDir/qsubcallsnp.sh\n";
print FINAL "echo -e '\\t'  generate snp result done at `date +'%Y-%m-%d %H:%M:%S %z'`\n\n";
print FINAL "#call  indel and VQSR \n";
print FINAL "perl $Bin/qsub-sge.pl  --maxjob 100 --convert no --reqsub  --queue $queue --P $project --resource vf=`du -b $GENOME | awk '{sum += \$1} END {printf(\"%.2fG\", sum / 1024 ^ 3*20)}'` --jobprefix CALLINDEL $CALLINDELshellDir/qsubcallindel.sh\n";
