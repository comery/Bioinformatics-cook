...
WARNNING:
         No configure file!
         So using /hwfssz1/ST_DIVERSITY/USER/yangchentao/perlscript/deal_genebank/mitogenome.conf
#------------------------------------------------------------------------------------------------------
_WARN_

if (! $configure) {
	print STDERR "$warn";
	$configure = "/hwfssz1/ST_DIVERSITY/USER/yangchentao/perlscript/deal_genebank/mitogenome.conf";
}
...
my $depth_file = abs_path($conf{'depth_file'}) if ($conf{'depth'});
		...
		#	print TRNA "mt1\t$start\t$end\n";
			...
			if ($feature->has_tag('gene')) {
				for $val ($feature->get_tag_values('gene')){
					...
					my ($trn,$type) = ($1,$2) if ($val =~ /(\w+)\((\w+)\)/);
					print TEXT "mt1\t$start\t$end\t$trn-$type\n";
				}
			}else{
...
print "$locus Reading genebank file done. \n";
print "$locus Creating circos.conf.\n...\n";
