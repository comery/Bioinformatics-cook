...
#		"Identity\tPositive\tGap\tAlign_length\tScore\tE_value\tQuery_annotation\tSubject_annotation\n" unless(defined $nohead);
	print "Query_id\tSubject_id\tIdentity\tAlign_length\tMiss_match\tGap\tQuery_start\tQuery_end\tSubject_start\tSubject_end\tE_value\tScore\tSubject_annotation\n" unless(defined $nohead);
	my $type=<BLAST>;
	$type=~s/\s+.+//s;	#blast的类型
		...
		for (my $i=0;$cycle[$i];$i++) {
			...
			for (my $j=0;$cycle_inner[$j];$j++) {
				...
				if ($cycle_inner[$j]=~/(\S+) bits.+?Expect.+?=\s+(\S+).+?Identities\D+(\d+)\/(\d+)\s+\((\S+)\%\)/s) {
					...
					$pointer->[$i][$j]{align_len}=$4;
					$pointer->[$i][$j]{identity}=$5;
					$pointer->[$i][$j]{mismatch} = $4 - $3;
				...
				print "$query\t$subject\t$pointer->[$i][$j]{identity}\t$pointer->[$i][$j]{align_len}\t$pointer->[$i][$j]{mismatch}\t",
					"$pointer->[$i][$j]{gap}\t$pointer->[$i][$j]{q_start}\t$pointer->[$i][$j]{q_end}\t$pointer->[$i][$j]{s_start}\t$pointer->[$i][$j]{s_end}\t",
					"$pointer->[$i][$j]{e_value}\t$pointer->[$i][$j]{score}\t$subject_annotation\n";
			}
		}
		if( ($gethit == 0 ) && ($subject_annotation eq '')){
			...
			for (my $j=0;$cycle_inner[$j];$j++) {
				...
				if ($cycle_inner[$j]=~/(\S+) bits.+?Expect.+?=\s+(\S+).+?Identities\D+(\d+)\/(\d+)\s+\((\S+)\%\)/s) {
					...
					$pointer->[0][$j]{align_len}=$4;
					$pointer->[0][$j]{identity}=$5;
					$pointer->[0][$j]{mismatch} = $4 - $3;
				...
				print "$query\t$subject\t$pointer->[0][$j]{identity}\t$pointer->[0][$j]{align_len}\t$pointer->[0][$j]{mismatch}\t",
					"$pointer->[0][$j]{gap}\t$pointer->[0][$j]{q_start}\t$pointer->[0][$j]{q_end}\t$pointer->[0][$j]{s_start}\t$pointer->[0][$j]{s_end}\t",
					"$pointer->[0][$j]{e_value}\t$pointer->[0][$j]{score}\t$subject_annotation\n";
			}
		}
	}
	$/="\n";
